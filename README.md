...
👉 [노션 바로가기](https://www.notion.so/2-342825a88d4480a596a1e0049161c2ee#34c825a88d4480ce8360f74671527c99)
...
```mermaid
flowchart TD
    A(["앱 시작 main.jsx"]) --> B["Provider 세팅 App.jsxAuthProvider / YumiProvider / TasteProfileProvider"]
    B --> C{"로그인 상태 확인Private Route - useAuth"}
    C -->|"미로그인"| D["/login Login 페이지"]
    C -->|"로그인됨"| E["세션 복원localStorage → yumpick_current_user"]
    D --> F["/register Register 페이지"]
    ...
    J --> J1["Header / ProfileCardKakaoMap / RestaurantListFloatingActions / NavigationBar"]
    J1 --> J2["위치 획득navigator.geolocation → lat/lng"]
    J2 --> J3["장소 추천 Kakao Places API"]
    J3 --> J4["최시 목록 표시"]
    J4 --> J5["백그라운드 이미지"]
    J1 --> BOT["AIChatbox 열기(독립 동작)"]
    J1 --> J6["AIRecommendationPanel일괄 AI 추천"]
    J6 --> J7["Python Flask API 호출"]
    J7 --> J8["Selenium 크롤링"]
    J8 --> J9["신뢰 랭킹 분석"]

    K --> K1["Friends 목록userFriends(user) → localStorage"]
    K1 --> K2["친구 ID 검색 FriendSearchBar"]
    K2 --> K3["findUserById"]
    K3 --> K4["친구 추가 성공tasteProfile 생성 + 저장"]
    K --> K5["/friends/:id FriendDetail 페이지"]
    K5 --> K6["FriendProfileCardTasteRadar 레이더 차트 표시"]
    K6 --> K7["FriendRecommendSectiontasteDatabase → 음식 추천"]
    ...
    L4 --> L5["설명 완료 버튼updateTasteProfile(form)"]
    L5 --> L6["useEffect → localStorage"]
    L1 --> L7["초기화 버튼 resetTasteProfile"]
```
