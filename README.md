...
| 문제 | 원인 | 해결 |
| --- | --- | --- |
| 상세보기 눌러도 반응 없음 | `let trustedRating` 변수 선언 누락 | 변수 선언 추가 |
| 카카오맵 팝업 차단 | `window.open` 브라우저 차단 정책 | `<a>` 태그 동적 생성 방식으로 교체 |
| AI 추천 항상 같은 점수 | 카테고리 구분 없이 단일 샘플 리뷰 사용 | 카테고리별 샘플 리뷰 분리 (중식/일식/카페/고기 등) |
| 식당 목록 느린 로딩 | 이미지 크롤링이 목록 표시 차단 | 목록 즉시 표시 후 이미지 백그라운드 순차 로딩 |
| 무한 렌더링 발생 | `onAnalyzed` 함수가 매 렌더마다 새로 생성 | `useRef`로 감싸고 `restaurantIds` 문자열로 비교 |
...
| 날짜 | 주요 작업 |
| --- | --- |
| 04.21 (월) | 프로젝트 킥오프, 역할 분담, 초기 세팅, Ollama 연동 확인 |
| 04.22 (화) | UI 고도화, Python Flask 서버 구축, 이미지 크롤링 설계, AI 챗봇 UI |
| 04.23 (수) | AI 리뷰 신뢰도 분석 구현, 입맛 일치도 계산, 친구 탭 1차 구현, 마이다이닝 구현 |
| 04.24 (목) | 브랜치 통합, 무한렌더링 해결, Profiler 최적화, 코드 스플리팅, 신뢰반영 별점 전체 적용 |
| 04.25 (금) | 최종 점검, 트러블슈팅 정리, 발표 준비, QA |
