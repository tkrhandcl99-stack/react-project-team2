...
flowchart TD
    ...
    K --> K1["일괄처리 AIRecommendationPanel"]
    K1 --> K2["Python Flask API 호출"]
    K2 --> K3["Selenium 크롤링"]
    K3 --> K4["신뢰 랭킹 분석"]
    K --> K5["Friends 목록userFriends(user) → localStorage"]
    K5 --> K6["친구 ID 검색 FriendSearchBar"]
    K6 --> K7["findUserById"]
    K7 --> K8["친구 추가 성공tasteProfile 생성 + 저장"]

    M --> M1["/friendsList"]
    M1 --> M2["FriendProfileCardTasteRadar 레이더 차트 표시"]
    M2 --> M3["FriendRecommendSectiontasteDatabase → 음식 추천"]
