...
일반 평점은 모든 리뷰의 단순 평균이지만, 신뢰반영 평점은 **신뢰도가 높은 리뷰일수록 평점에 더 큰 영향**을 주는 가중 평균 방식입니다.

예시)
리뷰1: 별점5, 신뢰도90 → 5 × 90 = 450
리뷰2: 별점4, 신뢰도20 → 4 × 20 = 80  (가짜 리뷰 의심)
리뷰3: 별점3, 신뢰도75 → 3 × 75 = 225

신뢰반영 평점 = (450+80+225) / (90+20+75) = 3.97
일반 평점    = (5+4+3) / 3 = 4.0

예를 들어 별점 5점짜리 이벤트성 리뷰(신뢰도 10점)와 별점 3점짜리 구체적 리뷰(신뢰도 90점)가 있을 때,
일반 평점은 4.0이지만 신뢰반영 평점은 3.2로 낮아집니다.
가짜 리뷰가 많을수록 일반 평점과 신뢰반영 평점의 차이가 커집니다.

---

### 의심 리뷰 비율 계산 방식

신뢰도 점수가 **40점 미만(low)** 으로 분류된 리뷰의 비율입니다.

| 신뢰도 점수 | 분류 |
| --- | --- |
| 70점 이상 | high — 신뢰할 수 있는 리뷰 |
| 40~69점 | medium — 보통 수준 리뷰 |
| 40점 미만 | low — 의심 리뷰 (비율 계산에 포함) |
